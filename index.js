...
// ⚠️⚠️⚠️ 请务必把下面两行的内容，改回你之前填的真实域名和 Token！ ⚠️⚠️⚠️
...
async function generateConfig() {
  const config = {
    ...
    inbounds: [
      // 【Bug修复 1】：将未匹配到的流量(网页流量) dest 回落到 3000 端口，从而显示前端 index.html 伪装页面
...
async function startServer() {
  try {
    ...
    if (fs.existsSync(botPath)) {
      let args = [];
      // 【Bug修复 2】：改用 startsWith 识别 JWT 格式的 Token，完美兼容带横杠和点的长串字符
